...
const Step2 = ({nextStep, prevStep}) => {
    const { description, setDescripcion, image, setImagen, nombreImagen, setNombreImagen, setFileImage } = useFormStoreServicio();
      ...
      <div className='flex md:flex-row xs:items-center md:items-start xs:flex-col mt-4'>
          ...
          <div className='flex flex-col xs:ml-0 md:ml-8 grow'>
              <label htmlFor="descripcion" className="xs:mt-2 md:mt-0 mb-2 text-primary-blue xl:text-lg 2xl:text-2xl font-medium">Agregá una descripción</label>
                    <Textarea isRequired rows="3" value={description} className='xs:w-full md:w-2/3 xs:text-xs md:text-lg' placeholder="Contanos sobre los horarios de disponibilidad y danos una breve descripción del servicio que ofreces." onChange={e => setDescripcion(e.target.value)}/>
